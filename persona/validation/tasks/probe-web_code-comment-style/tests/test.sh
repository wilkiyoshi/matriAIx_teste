...
if not path.is_file():
    print("missing fizzbuzz.py",file=sys.stderr); sys.exit(1)
...
(vd/"structured_output.json").write_text(json.dumps({
  "schemaVersion":"1.0","artifactType":"matraix.trial_evaluation","taskType":"web",
  "presenceCheck":{"passed":True,"requiredArtifacts":["fizzbuzz.py"],"missingArtifacts":[]},
